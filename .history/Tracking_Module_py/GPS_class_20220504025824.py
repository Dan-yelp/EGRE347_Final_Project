...
class GPS:
    ...
    def read_byte(self,e):
        ...
        #State 1 - Receives message type into a string, and only accepts one of the formentioned types
            ...
            #Excludes all but type of concern for brevity
        ...
        #State 4 - Output message to terminal, option for wanted type, this is where you can save wanted data, some examples are commented here
        if((self.State == "Writing") & (self.Type_string == "GPGGA") & (len(self.Data_string) == 69)):
            # print('length of data_string:',len(self.Data_string))
            # print('Message type:',self.Type_string)
            # print('Message data:',self.Data_string)
            # print('Message checksum:',self.Check_string,'\n')
            self.long_string = self.Data_string[16:26]
            self.lat_string = self.Data_string[28:39]
            self.tag_data = self.Data_string[6:16] + self.Data_string[16:26] + self.Data_string[28:39]
            print('Tag data:', self.tag_data + self.lat_string + self.long_string)
            # print('longitude:',self.long_string)
            # print('latitude:',self.lat_string)
