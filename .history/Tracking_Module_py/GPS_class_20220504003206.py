...
class GPS:
    ...
    def read_byte(self,e):
        ...
        #State 4 - Output message to terminal, option for wanted type, this is where you can save wanted data
        if((self.State == "Writing") & (self.Type_string == "GPGGA"))# & (len(self.Data_string) == 64)):
            print('length of data_string:',stringlen(self.Data_string))
            # print('Message type:',self.Type_string)
            # print('Message data:',self.Data_string)
            # print('Message checksum:',self.Check_string,'\n')
            self.long_string = self.Data_string[13:26]
